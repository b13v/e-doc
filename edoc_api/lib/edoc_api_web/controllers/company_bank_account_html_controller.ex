defmodule EdocApiWeb.CompanyBankAccountHTMLController do
  ...
  def show(conn, %{"id" => id}) do
    ...
    with {:ok, _company} <- require_company(user.id),
      ...
      render(conn, :show, account: account, page_title: gettext("Bank Account"))
    else
  ...
  def edit(conn, %{"id" => id}) do
    ...
    with {:ok, _company} <- require_company(user.id),
         {:ok, account} <- Payments.get_company_bank_account_for_user(user.id, id) do
      render(conn, :edit, account: account, page_title: gettext("Edit Bank Account"))
    else
      {:error, :company_required} ->
        conn
        |> put_flash(:error, gettext("Please set up your company first."))
    ...
    with {:ok, _company} <- require_company(user.id),
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        case Payments.get_company_bank_account_for_user(user.id, id) do
          {:ok, account} ->
            conn
            |> put_flash(:error, ErrorHelpers.format_changeset_errors(changeset))
            |> render(:edit,
              account: account,
              changeset: changeset,
              page_title: gettext("Edit Bank Account")
            )
