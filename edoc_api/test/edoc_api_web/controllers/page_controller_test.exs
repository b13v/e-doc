defmodule EdocApiWeb.PageControllerTest do
  ...
  describe "GET /" do
    test "renders landing auth links for guests", %{conn: conn} do
      ...
      refute body =~ ~s(href="http://localhost:4000/login")
      assert body =~ ">Регистрация<"
      assert body =~ ~s(href="/signup")
      refute body =~ ~s(href="http://localhost:4000/signup")
      assert body =~ ~s(data-auth-link="login")
      assert body =~ ~s(data-auth-link="signup")

      assert body =~
               ~r/<a href="\/signup"[^>]*class="btn btn-primary hero-btn"[^>]*>[\s\S]*Начать бесплатно[\s\S]*<\/a>/

      assert body =~
               ~r/<a href="\/login"[^>]*class="btn btn-secondary"[^>]*data-i18n="pricing.starter_cta">Выбрать тариф<\/a>/

      assert body =~
               ~r/<a href="\/signup"[^>]*class="btn btn-white"[^>]*data-i18n="pricing.base_cta"[^>]*>Начать 14-дневный тест<\/a>/

      assert body =~
               ~r/<a href="\/signup"[^>]*class="btn btn-white"[^>]*>[\s\S]*Зарегистрироваться бесплатно[\s\S]*<\/a>/
      ...
      assert body =~ "'[data-auth-link=\"signup\"]'"
      ...
      assert body =~ "'[data-auth-link=\"login\"]'"
      assert body =~ ~s(<link rel="stylesheet" href="/assets/landing.css" />)
      refute body =~ "<style>"
      assert body =~ ~s(aria-expanded="false")
      assert body =~ "header.classList.toggle('mobile-menu-open'"
      refute body =~ "nav.style."
      ...
      assert body =~ "Все из тарифа Стартовый"
      refute body =~ "Все из тарифа Базовый"
      ...
      assert body =~ "Автоматическая валидация БИН"
      refute body =~ "валиадация"
      ...
      assert landing_css =~ ".footer-link-disabled"
      assert landing_css =~ ".header.mobile-menu-open .nav"
